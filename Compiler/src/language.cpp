...
bool Language::SetupEnvironment(const char* source) {
	NativeSymbols::Copy(env_->terminalSymbols, env_->nonterminalSymbols);

	if (!ParseProductions(source)) {
		return false;
	}
...
bool Language::ParseProductions(const char* source) {
	TextScanner textScanner;
	GrammarReader reader(source);
...
void Language::BuildSyntaxer(const char* source) {
