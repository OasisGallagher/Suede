...
namespace ImGui
{
    ...
    IMGUI_API void          Indent(float indent_w = -1);                                  // move content position toward the right, by style.IndentSpacing or indent_w if != 0
    IMGUI_API void          Unindent(float indent_w = -1);                                // move content position back to the left, by style.IndentSpacing or indent_w if != 0
	...
	IMGUI_API bool          DragUInt(const char* label, unsigned* v, float v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* format = "%d");                                       // If v_min >= v_max we have no bound
	...
	IMGUI_API bool          __IsItemDoubleClicked(int mouse_button = 0);
    ...
    IMGUI_API void          SaveIniSettingsToDisk(const char* ini_filename, ImGuiContext* ctx);
    IMGUI_API const char*   SaveIniSettingsToMemory(size_t* out_ini_size, ImGuiContext* ctx);               // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.
...
struct ImGuiIO
{
	...
	void* renderDrawListsFnParam;
    void        (*RenderDrawListsFn)(ImDrawData* data, void* param);
#else
