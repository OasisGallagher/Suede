...
void GLSLParser::AddConstants() {
	...
	globals_ += "#define C_MAX_ENTITY_MATRICES " + std::to_string(GLLimits::Get(GLLimitsMaxUniformBlockSize) / sizeof(glm::mat4))  + "\n";
}
...
bool GLSLParser::PreprocessInclude(const std::string& parameter) {
	...
	if (!FileSystem::ReadAllText(Resources::GetRootDirectory() + "shaders/" + path, source)) {
		return false;
	}
