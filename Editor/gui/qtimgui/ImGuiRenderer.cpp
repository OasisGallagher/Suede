...
static QHash<int, ImGuiKey> keyMap = {
...
static QByteArray currentClipboardText;
...
void ImGuiRenderer::initialize(ImGuiWindowWrapper *window, ImFontAtlas* fontAtlas) {
	...
	io.GetClipboardTextFn = [](void *user_data) {
		Q_UNUSED(user_data);
		currentClipboardText = QGuiApplication::clipboard()->text().toUtf8();
		return (const char *)currentClipboardText.data();
	};
...
void ImGuiRenderer::onMouseEvent(QMouseEvent *event) {
	ImGui::SetCurrentContext(m_context);
...
void ImGuiRenderer::onWheelEvent(QWheelEvent *event) {
	ImGui::SetCurrentContext(m_context);
...
#include <QDebug>
void ImGuiRenderer::onKeyEvent(QKeyEvent *event) {
	ImGui::SetCurrentContext(m_context);
	ImGuiIO& io = ImGui::GetIO();
	...
	if (keyMap.contains(event->key())) {
		io.KeysDown[keyMap[event->key()]] = keyDown;
	}
	...
	qDebug() << this << " " << this->m_window->widget()->hasFocus() << " " << &io << " " << keyDown;
...
bool ImGuiRenderer::eventFilter(QObject *watched, QEvent *event) {
	if (m_window->widget() == watched) {
		...
		case QEvent::MouseButtonDblClick:
			...
			ImGui::SetCurrentContext(m_context);
			this->onMouseEvent(static_cast<QMouseEvent *>(event));
			break;
		case QEvent::Wheel:
			ImGui::SetCurrentContext(m_context);
			this->onWheelEvent(static_cast<QWheelEvent *>(event));
			break;
